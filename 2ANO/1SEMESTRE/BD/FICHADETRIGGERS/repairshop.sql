...
DROP TRIGGER IF EXISTS updatePartStock;
CREATE TRIGGER updatePartStock AFTER INSERT ON RepairPart FOR EACH ROW BEGIN
    UPDATE Part
    SET availableQuantity = availableQuantity - New.quantity
    WHERE Part.partId = New.partId;
END;

DROP TRIGGER IF EXISTS updateCarOwner;
CREATE TRIGGER updateCarOwner AFTER INSERT ON Repair FOR EACH ROW WHEN (New.clientId is NULL) BEGIN
    UPDATE Repair
    SET clientId = (SELECT clientId FROM Car WHERE idCar = New.idCar)
    WHERE repairId = New.repairId;
END;

DROP TRIGGER IF EXISTS validatePartsRepair;
CREATE TRIGGER validatePartsRepair BEFORE INSERT ON RepairPart FOR EACH ROW WHEN (NOT EXISTS (SELECT * FROM Repair 
    JOIN Car ON Repair.idCar = Car.idCar JOIN PartModel ON  Car.modelId = PartModel.modelId JOIN Part ON PartModel.partId = Part.partId
    WHERE Repair.repairId = New.repairId AND Part.partId = New.partId AND Part.availableQuantity >= New.quantity))
    BEGIN
        SELECT raise(ABORT, "Invalid part or Quantity");
END;

CREATE VIEW ModelsBrands AS SELECT Model.name AS modelName, Brand.name AS BrandName FROM Model, Brand WHERE Model.brandId = Brand.brandId;
DROP TRIGGER IF EXISTS insertModelBrand;
CREATE TRIGGER insertModelBrand INSTEAD OF INSERT ON ModelsBrands FOR EACH ROW BEGIN
    INSERT OR IGNORE INTO Brand (Name) VALUES (New.brandName)
    INSERT OR IGNORE INTO Model (Name, brandId) SELECT New.modelName, brandId FROM Brand WHERE Name = New.brandName;
END;